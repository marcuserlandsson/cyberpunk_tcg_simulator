...
// The invariant battery itself lives in a shared module so the Task 10 AI
// suite drives heuristic-vs-heuristic games through the SAME checks.
import {
  checkInvariants as checkInvariantsShared,
  checkLegalActionsSanity,
  VALID_END_REASONS,
...
// The turn bound itself (and its derivation) now lives alongside the rest of
// the invariant battery in ./invariantChecks.
...
// ---------------------------------------------------------------------------
// Invariant checks (./invariantChecks — shared with tests/ai/heuristic.test.ts)
// ---------------------------------------------------------------------------

function checkInvariants(state: GameState): string[] {
  return checkInvariantsShared(db, state)
}
