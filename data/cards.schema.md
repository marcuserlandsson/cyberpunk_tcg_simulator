...
§81 ff. from Task 8 batch 4, §92 ff. from Task 8 batch 5, §107 ff. from Task 8
batch 6, §120 ff. from Task 8 batch 7, §134 ff. from Task 8 batch 8, and
§141–§144 from the deferred slice that closed every remaining deferral — the
last of the 141 cards).

### `EffectDef`

| Field | Notes |
|---|---|
| `trigger` | `onPlay` \| `onCall` \| `onAttack` \| `onDefeat` \| `onBlock` \| `onWinFight` \| `onSpend` \| `onFriendlyStealDie` \| `onFriendlyAttack` \| `onUnitDefeated` \| `onRivalAdjustFriendlyGig` \| `onEndTurn` \| `onFriendlyEquippedSpend` \| `onLoseFight` \| `onStartTurn` \| `onFriendlyBlock` \| `onFriendlyCardPlayed` \| `activated` \| `static`. `onFriendlyStealDie` was added in Task 8 batch 1 (§42); the next four in batch 2 (§60); `onFriendlyEquippedSpend` in batch 3 (§68 ff.) — "When a friendly EQUIPPED Unit or Legend is spent, ..." (alt-cunningham-mother-of-daemons), fired from the same `spendOnDraft` seam as the self-referential `onSpend`. Batch 5 (§92 ff.) adds three more: `onLoseFight` — the mirror image of `onWinFight`, self-referential ("when THIS Unit loses a fight"), not a watcher; `onStartTurn` — the mirror image of `onEndTurn`, fired from a new `reduce.ts` helper wrapping `game.ts`'s `beginTurn` (kept out of `game.ts` itself to avoid a new import-cycle direction); `onFriendlyBlock` — a watcher version of `onBlock` ("when A FRIENDLY Unit uses {Blocker}", not just this card). All watcher triggers fire on every in-play card of whichever side the printed text names (or, for bare wording, both sides) rather than on the card that acted. Task 8 batch 6 adds no new trigger, but extends the *context* every non-`onPlay` trigger firing can carry with `TriggerContext.equipHostUid` (docs/rulings.md §107 ff.) — when `fireWatcherTrigger` fires a trigger for an attached Gear rather than the watching card itself, this carries the uid of the Unit/Legend wearing it, since `'self'` on the Gear's own `EffectDef` resolves to the Gear's own uid, not its host (`sandevistan`'s "ready this Unit or Legend"). Task 8 batch 7 (§120 ff.) adds `onFriendlyCardPlayed` — "When you play a BRAINDANCE Program, ..." / "The first time you play a Blue Unit or Blue Gear each turn, ..." — a watcher fired from `playCardOnDraft` right after the played card's own `onPlay`, carrying the played card's own color/type/tags in the context so the condition can narrow which plays qualify. Batch 7 fix round 1 (§133) adds `onFriendlyStealComplete` — "When a friendly Unit steals 1 or more Gigs, ..." — a watcher fired ONCE by `combat.ts`'s `takeStolenGig` when the whole steal episode it is resolving finishes, unlike the per-die `onFriendlyStealDie` every other card in the pool uses. The deferred slice (§143) adds `onGigRoll` — "When you roll a min or max value on a Gig, ..." (kerry-eurodyne-axe-attitude-audience) — a watcher fired wherever a Gig die's value is actually rolled (the start-of-turn fixer roll, the new `gigReroll` decision, and the `rerollGig` node), on the ROLLER's own in-play cards, carrying the die's size and the value it landed on. |
...
| `condition` | `{ streetCredAtLeast?, friendlyGigValueAtLeast?, rivalGigLeadAtLeast?, stolenDieSize?, streetCredAheadOfRival?, streetCredBelow?, duringOwnTurn?, sourcePowerAtLeast?, selfIsStealer?, attackerKeyword?, defeatedKeyword?, friendlyGigsAtLeastValueCount?, friendlyGigDistinctValuesAtLeast?, friendlyGigEvenAndOdd?, friendlyGigValueEquals?, streetCredDiffAtLeast?, sourceEquipped?, stealerIsLegend?, stolenDieValueParity?, defeatedIsFriendly?, defeatedWasEquipped?, streetCredParity?, allFriendlyLegendsFaceUp?, sourceSpent?, friendlyGigValuePair?, friendlyEquippedCountAtLeast?, sourceStoleGigThisTurn?, friendlyProgramNotPlayedThisTurn?, playedCardColor?, playedCardType?, playedCardKeyword?, stealerKeywordAnyOf?, streetCredBehindRival?, friendlyGigSizeAtMin? }` (this shape also lives on its own as `EffectCondition`, so `conditionalEffect` — see below — can carry one without an enclosing `EffectDef`) — "☆ N or more", "if you control a Gig with 8+ value", "if a Rival controls at least 2 Gigs more than you", the watcher-only die-size gate (§42, §50), "if you have more/less ☆ than a Rival" and "less than N ☆" (§55), "during your turn" (§59), the attacking Unit's own power (§61), "when THIS Unit steals" (§61), the attacking/defeated Unit's own faction-or-keyword tag (§60/§66), "2 or more Gigs with 8+ value" (§62), "2 or more Gigs with different values" (§68 ff.), "a Gig with an even value and a Gig with an odd value" (§68 ff.), "[a fixed number] equals the value of a friendly Gig" (§68 ff.), "☆ differs from a Rival's by N+" (§68 ff.), "if it's equipped" — does the SOURCE card itself carry attached Gear (§68 ff.), "a friendly LEGEND steals" and "if its value is even/odd" (§81 ff.), "a friendly EQUIPPED Unit is defeated" (§81 ff.), "☆ is an even/odd number", "all friendly Legends are face-up", "if THIS Unit is spent", "if you control a value-pair of Gigs" — two Gig dice sharing a value (§92 ff.), (Task 8 batch 6, §107 ff.) "if 5 or more friendly Units and/or Legends are equipped" (`friendlyEquippedCountAtLeast`), and (Task 8 batch 7, §120 ff.) "if this Unit stole a Gig this turn" (`sourceStoleGigThisTurn`), "unless you played a Program this turn" (`friendlyProgramNotPlayedThisTurn` — the first condition field checked for FALSE rather than true), the played card's own color/type/tag for `onFriendlyCardPlayed` (`playedCardColor`/`playedCardType`/`playedCardKeyword`), and an OR of stealer tags for `onFriendlyStealComplete` ("a CORPO or GANGER Unit steals", `stealerKeywordAnyOf` — moved from `onFriendlyStealDie` in fix round 1, §133, since the printed event is per steal episode, not per die). Task 8 batch 8 (§134 ff.) adds two more: "if you have less ☆ than a Rival" (`streetCredBehindRival` — the mirror of `streetCredAheadOfRival`, strictly less; modded-muramasa, mt0d12-flathead) and "if a friendly d[N] is a min Gig" (`friendlyGigSizeAtMin` — a friendly Gig die of exactly that size currently showing 1; pyramid-song). The deferred slice (§143) adds two `onGigRoll`-only facts: `rolledExtremeValue` ("a min or max value on a Gig" — the roll landed on 1 or on the die's own size) and `rolledDieSizeAnyOf` ("If it's a d20, ..." — an enumerated SET of die sizes, so kerry's "draw 3 instead" def names `[20]` and its sibling names the other five explicitly, with no negation primitive). |
...
Task 8 batch 6 adds one more (§107 ff.): `selfGear` — "a Gear from THIS
Legend/Unit" — the Gear attached to the source card itself, unlike
`friendlyGear` (every friendly Unit/Legend's Gear). Task 8 batch 7 adds one
more (§120 ff.): `friendlyHandOrTrashProgram` — the `friendlyHandOrTrashUnit`
shape (§63), baked to `'program'` instead of `'unit'` (lizzy-wizzy-delicate-
weapon).
...
unequipped?, spentOnly?, lowestPower?, readyOnly? }`,
...
(`maxPowerVsFriendlyD20`, §64), and (Task 8 batch 6, §107 ff.) "an
UNEQUIPPED friendly Unit" (`unequipped` — `attachedGear.length === 0`) and
bare "a spent Unit" (`spentOnly` — `!ready`, on the bare `anyUnit` spec), and
(Task 8 batch 7, §120 ff.) "a Rival's lowest-power Unit. (If there are
multiple, choose 1.)" (`lowestPower` — narrows to the candidates tied for
the RAW zone's own minimum `effectivePower`, les-e-le-mens), and (Task 8
batch 8, §134 ff.) "a **ready** friendly Unit" (`readyOnly` — the mirror
image of `spentOnly`, unlikely-bond). The deferred slice (§141) adds
"an **equipped** Unit" (`equipped` — `attachedGear.length > 0`, the mirror
image of `unequipped`; cyberpsychosis).

### Nodes

| Kind | Printed text it encodes |
|---|---|
| `draw` | "Draw N." `count` accepts the same `number \| DynamicAmount` as `buffPower`/`staticPower` (§68 ff.), e.g. "Draw 1 for each friendly Gig with an odd value", (§92 ff.) the bare string `'friendlyGigValuePairCount'` for "Draw 1 for each friendly value-pair of Gigs", or (Task 8 batch 6, §107 ff.) `'friendlyFaceUpLegendCount'` for "draw 1 for each friendly face-up Legend" (used inside a `scripted` node for its own "if you do" dependency, `panam-palmer-strength-through-family`). |
| `discardRandomRival` | "A Rival discards N at random." |
| `buffPower` | "Give a … +N power this turn / permanently". `amount` may be the string `'friendlyMaxGig'` for "power equal to a friendly max Gig" (§39), `{ perEquippedGear: N }` for "+N power for each of its equipped Gear" (§59), `{ perFriendlyGigParity: { parity, amount } }` for "+N power for each friendly Gig with an even/odd value" (§68 ff.), or (batch 6) `'friendlyFaceUpLegendCount'`. |
...
| `chooseOne` | "Choose one effect. A // B". `chooser`: `'controller'` (default), `'rivalIfBehindStreetCred'` ("If you have less ☆ than a Rival, they instead choose one for you"), or `'allUnlessBehindStreetCred'` (every mode resolves unless you are behind, then the rival picks one — `gunpoint-diplomacy`, §45/§54). Task 8 batch 8 (§134 ff.) adds an independent `allIf?: EffectCondition` field, checked ahead of `chooser`: "Choose one effect. If a friendly d4 is a min Gig, choose both instead." (`pyramid-song`) — the mirror image of `allUnlessBehindStreetCred`, generalized to an arbitrary board condition rather than a second street-cred-specific chooser value. |
...
| `costReduction` | *static*: "Play this for -1 €$ for each friendly Gig with 8+ value, to a minimum of 1 €$" (§44), (§81 ff.) "-1 €$ for each Unit in your trash" — a second `CostReduction.per` variant (`'unitInTrash'`) with no `value` threshold, just a flat count (`trauma-team-operatives`) — or (Task 8 batch 6, §107 ff.) "-1 €$ for each friendly face-up Legend" — a third variant, `'friendlyFaceUpLegend'` (`zetatech-berserk`). All variants apply only to the card's OWN play cost. |
...
| `matchGig` | "Set a Gig's value to the value of another Gig" (Task 8 batch 6, §107 ff.) — two fixed-order `anyGigDie` slots (the die being overwritten, then the die being read from); copies the second's value onto the first, clamped to `[1, size]` like `changeGig`. Fires `onRivalAdjustFriendlyGig` on the overwritten die's owner, mirroring `changeGig`/`swapGig`. |
| `freeLegendCall` | *static*: "During your turn, you may Call a Legend for free" (`panam-palmer-strength-through-family`, §107 ff.) — zeroes the printed 1 €$ Call-a-Legend cost for the controller while the owning `EffectDef`'s `condition` holds (typically `duringOwnTurn`). Read by `economy.legendCallCost`. |
| `goSoloTax` | *static*: "Rivals must pay +N €$ to use {Go Solo}" (`riot-shield`, §107 ff.) — the mirror image of `rivalCantAttackWhenPlayed`: a tax on the OPPOSING side's {Go Solo} plays, read by `effects.goSoloPayment` and `reduce.ts`'s `isLegal`. |
| `attackPowerBonus` | *static*: "Friendly ARASAKA Units have +1 power while attacking" / "Other friendly Units have +2 power while attacking" (§107 ff.) — a power bonus that exists only for the duration of the MATCHING Unit's own attack (fight power and Gig-steal power alike), never folded into `effectivePower`. `keyword` matches `cardTags` (role tag or the single faction tag), not `TargetFilter.keyword` (`hasKeyword`-only). `excludeSelf` excludes the printing card itself. |
| `attackUnitDespiteLag` | *static*: "This Unit can attack rival Units the turn it's played" (`sandayu-oda-hanako-s-guardian`, §107 ff.) — the mirror image of `attackGigAreaDespiteLag`: unlocks ONLY a rival Unit target despite Lag, never the Gig area. |
| `buffFightPower` | "{Quick} A friendly Unit has +N power for each friendly face-up Legend while fighting rival Units this turn" (`synapse-burnout`, §107 ff.) — an until-end-of-turn, FIGHT-ONLY power bonus on the chosen target, stored on `CardInstance.fightPowerBonusThisTurn` and read only by `query.fightPowerBonus`, never `effectivePower`. `amount` accepts the same dynamic amounts as `buffPower`. |
| `readyEddies` | "ready N Eddie(s)" (Task 8 batch 7, §120 ff. — `delamain-cab`, `dying-night-v-s-pistol`, `evelyn-parker-beautiful-enigma`, `misty-olszewski-mender-of-broken-spirits`) — readies up to `count` SPENT cards in the controller's own Eddies zone. No target slot: every Eddie is worth exactly 1 €$ when ready regardless of which card it is, so which spent one is readied first is not a decision. |
| `cantBeBlocked` | *static*: "This Unit can't be blocked" (`mt0d12-flathead`, Task 8 batch 8, §134 ff.) — the mirror image of `cantAttack` on the defending side: `combat.ts`'s `reactActions` drops every `{Blocker}` reaction for the current pending attack's attacker while this static is active. |
| `floatingEffect` | An effect that outlives its own resolution, written into `GameState.floatingEffects` (the deferred slice, §141). Its `floating` field is a strict `FloatingSpec` union — one variant per printed clause, each with its own printed `expiry` (`endOfTurn` for "this turn"/"at the end of this turn", lapsing with `tempPower` in `clearTurnBuffs`; `ownerNextTurnStart` for "until your next turn"/"next turn", lapsing in `beginTurn` for its controller only). The variants: `rivalStealCappedByPower` (chrome-fang), `winFightMarginSteal` (appetite-for-destruction), `loseFightDefeatFoe` (safety-override), `rivalFightNoDefeat` (reboot-optics), `defeatIfActed` (cyberpsychosis), `unitCantAttack` (chrome-reverie), `mustAttack` (mox-inciters, evelyn-parker-beautiful-enigma, §142). The last three carry a `target`/`filter` pair and so reserve one ordinary target slot — `'chosen'` inside a `sameTarget` for cyberpsychosis's "**that** Unit", a real enumerated `rivalUnit` decision for the other two. A "the next time ..." entry needs no spent flag: whoever consumes it removes it. |
| `gigRerollOption` | *static*: "When you roll in a Gig from your fixer area, you may ignore the result and reroll it once" (`kerry-eurodyne-axe-attitude-audience`, §143) — a PERMISSION, read by `query.friendlyGigRerollOption`, which makes the gig-gain step stop in the new `gigReroll` phase for a real two-option decision (`Action` `chooseGigReroll`) instead of going straight to `main`. |
| `defeatInterceptSelf` | *static*: "If a friendly Unit would be defeated, you may spend N €$ to defeat this Legend instead. (Remove it from the game.)" (`jackie-welles-mama-s-favorite`, §144) — a would-be-defeated INTERCEPTION, unlike `defeatShield` (§46) which is unconditional, costless and needs no decision. Read by `query.defeatInterceptorFor` at the single interception seam in `combat.defeatUnit`, which every defeat path funnels through. |
| `stealInterceptByDiscard` | *static*: "When a rival Unit would steal a Gig, you may discard 1 with cost equal to that Gig's value. If you do, the Gig isn't stolen." (`alt-cunningham-mother-of-daemons`'s second clause, deferred by §72 and finished in §144) — the same interception family, on the steal mutation instead of the defeat one, asked in `combat.takeStolenGig` before the die moves. |

The deferred slice also adds engine state and decisions that are NOT card-data
vocabulary but that every consumer of `GameState` needs to know about
(docs/rulings.md §141-§144): `floatingEffects` (the zone above),
`pendingGigRoll` + the `gigReroll` phase + the `chooseGigReroll` action (§143),
and `pendingIntercept` + `interceptAnswers` + the `intercept` phase + the
`answerIntercept` action (§144 — a would-be-mutation decision, implemented by
rolling the whole action back and replaying it once the answer is in, which is
deterministic because the rng lives in the replayed state).
