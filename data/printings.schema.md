...
Reviewed artwork metadata is preserved for unchanged printing/image identities.
See [artwork identity](../docs/artwork-identity.md) before editing those mappings.
...
| Field | Type | Meaning |
|---|---|---|
| `artworkId` | `string?` | Stable reviewed illustration ID from `artworks.json`; absent means awaiting review. |
| `playable` | `boolean?` | False for collection-only showcase objects that cannot supply gameplay-copy totals. |
| `sourceImageUrl` | `string?` | Unsigned source-image provenance; changes invalidate the artwork mapping during import. |
...
- Run date: 2026-09-08
- Row count: 460 printings
- Card coverage: 151/151 cards in `data/cards.json`, each with ≥1 printing
- Distinct sets: 13 (`edgerunneropens1`, `arasakademodeck`, `boxtoppersbeta`, `boxtoppersretail`,
