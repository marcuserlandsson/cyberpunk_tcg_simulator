...
### 08n — source lifetime and contextual timing

Pending effects now retain last-known source/host information after field exits; board-state conditions read the state at resolution rather than deciding whether to queue the trigger. Attack/fight power applies throughout the relevant pending window. Start-boundary durations expire after pending effects and preserve newly created durations. Gig identities survive rolls/returns and keep delayed stolen-die effects attached to the correct die. Private Legend peeks show readable RAM limits. Verified 1,101 engine/card/AI/fuzz checks across the main run and a targeted rerun, TypeScript/build, and four production-preview browser gameplay checks. Two 40-game AI regressions exceeded their default five-second timeout; gave them an explicit 60-second allowance and reran with unchanged assertions.

### 08o — optional instructions and search failure

Added explicit accept/decline instructions for Yorinobu/Lizzy free plays, Gilded Matón/Heywood Gear defeats, MaxTac AV's swap, Meredith's retrieval, Peace Offering's value change, and Bonnie and Clyde's second defeat. The Heist and Judy Braindance Maestro also offer their printed alternatives. Declining skips only that instruction, preserving later mandatory clauses. Sketchy Ripper permits failing its filtered hidden search. Verified 701 engine/card/data tests, three AI hidden-information checks and TypeScript. Shattered Memories' optional exact draw and rival-chosen discards remain in the next checkpoint alongside terminal draw handling.
